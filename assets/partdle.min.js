...
function W(a){if("undefined"!=typeof a&&null!=a)if("undefined"==typeof a.O&&"undefined"!=typeof a.play)a.load(),a&&0<a.currentTime&&!a.paused&&!a.ended&&2<a.readyState||a.play();else if(!(a&&0<a.currentTime&&!a.paused&&!a.ended&&2<a.readyState)){var b=a.createBufferSource();b.buffer=a.O;b.connect(a.destination);b.start()}}
...
function Ta(a){var b="zh-TW";"string"!=typeof b&&(b="en-US");if(!/https:\/\//.test(a)){a=a.replace(/\.tts$/,"");/\.(\w{2}\-\w{2})$/.test(a)||(a+="."+b);var c=b="";/\.(\w{2}\-\w{2})$/.test(a)&&(a=window.RegExp.leftContext,c=window.RegExp.lastParen);isNaN(1)||(b="https://translate.google.com/translate_tts?ie=UTF-8&tl=${tts_language}&client=tw-ob&ttsspeed=${tts_speed}&q=".replace(/\$\{tts_language\}/i,""==c?"zh-TW":c),b=b.replace(/\$\{tts_speed\}/i,(1).toString()),b+=encodeURIComponent(a));a=b}"undefined"==
typeof I||null==I?I=Ra(a,!0):I.src=a;I.ha=[];W(I)}
function Ua(){function a(e){var g=document.getElementById("practiceSVG").getScreenCTM();if(e.touches){for(var k=e.changedTouches[0].identifier,l=-1,m=0;m<e.touches.length;m++)if(e.touches[m].identifier==k){l=m;break}0>l&&(l=0);e=e.touches[l]}return{x:(e.clientX-g.e)/g.a,y:(e.clientY-g.f)/g.d}}function b(e){var g=-1;if("undefined"!=typeof e.changedTouches&&null!=e.changedTouches&&0<e.changedTouches.length){e=e.changedTouches[0].identifier;for(var k=0;k<h.length;k++)null!=h[k].id&&h[k].id==e&&(g=k)}else g=
...
penUp)}catch(e){}if("undefined"!=typeof penUp)try{f.removeEventListener("touchleave",penUp)}catch(e){}if("undefined"!=typeof penUp)try{f.removeEventListener("touchcancel",penUp)}catch(e){}var h=[];penDown=function(e){Sa();var g=e.target;if("path"==g.tagName||g.classList.contains("pathBg")||g.classList.contains("wordNG")){e.preventDefault();e.stopPropagation();var k={};"undefined"!=typeof e.changedTouches&&null!=e.changedTouches&&0<e.changedTouches.length&&(k.id=e.changedTouches[0].identifier);k.target=
...
d.setAttribute("class","message"),c.appendChild(d);d.setAttribute("style","opacity:0;")}function Va(a){var b=0,c=document.querySelector("#practiceSVG");c&&(b=Math.floor((c.viewBox.baseVal.width-2048*Q.length-40*(Q.length-1))/2)+2088*a);return{x:b,y:1024}}
function Wa(){var a,b=document.querySelector("#practiceSVG");if(b)for(var c=0;c<b.children.length;c++){var d=b.children[c],f=d.getAttribute("id");"g"==d.tagName&&"string"==typeof f&&(a=f.match(/([^-]+)-(\d+)-(\d+)/))&&(f=Va(Number(a[2])),d.setAttribute("transform","translate("+f.x+" "+f.y+")"))}}
function Xa(){var a=document.querySelector("#practiceSVG");if(a)for(var b=0;b<Q.length;b++){var c=Va(b);c=V("g",{transform:"translate("+c.x+" "+c.y+")","stroke-width":3});var d="";d+='<rect width="2048" height="2048" fill="none" stroke="green" rx="150" ry="150" />\n';d+='<line x1="0" x2="2048" y1="1024" y2="1024" stroke="red" stroke-dasharray="50" />\n';d+='<line y1="0" y2="2048" x1="1024" x2="1024" stroke="red" stroke-dasharray="50" />\n';c.innerHTML=d;a.appendChild(c)}}
function Ya(a){var b,c,d,f,h,e,g,k,l,m,p,n,q,w,B,v,t,z,C,F;pa(function(K){if(1==K.g){if(!(0<P.length)){"function"==typeof a&&(Za(),a());K.g=0;return}P.length==Q.length&&(window.J=Array.from({length:$a(Q)},function(G,y){return y}));b=Math.floor(Math.random()*P.length);c=P[b];P.splice(b,1);d=Q[c];return ia(K,Ma(d),3)}f=K.j;h=Ia(d,f);e=new DOMParser;g=e.parseFromString(h,"text/html");k=ab(d);l=document.querySelector("#practiceSVG");m=l.viewBox.baseVal;p=2048*c;n="translate("+p+" 1024)";q=g.querySelectorAll("g");
...
y=(500>H?1.5:1351.68>H?1:.75)*H/2;var Ea=(500>D?1.5:1351.68>D?1:.75)*D/2;M+=H/2;N+=D/2;H="fill:#ffffff;fill-opacity:0.001; stroke:none;stroke-opacity:0;";"1"==X("debug")&&(H="fill:#ffff00;fill-opacity:0.1; stroke:none;stroke-opacity:0;");y=V("ellipse",{"class":"pathBg",cx:M,cy:N,rx:y,ry:Ea,style:H});G=G.parentElement;G.insertBefore(y,G.children[0])}),k&&k[v]&&q[v].setAttribute("cns",k[v]),t=q[v].getBBox(),$a(Q),b=Math.floor(Math.random()*window.J.length),z=window.J[b],window.J.splice(b,1),C=m.width/
2-t.width/2-t.x,F=m.height/2-t.height/2-t.y,q[v].setAttribute("transform","translate("+C+" "+F+")"),q[v].setAttribute("fill",u[z%u.length]);Ya(a);K.g=0})}
function Za(){var a=document.querySelector("#practiceSVG");if(a){var b=a.viewBox.baseVal,c=b.width/$a(Q),d=Array.from({length:$a(Q)},function(p,n){return n});a=a.querySelectorAll("g");for(var f=0;f<a.length;f++){var h=a[f];if(/[^-]+-\d+-\d+/.test(h.getAttribute("id"))){var e=h.getBBox(),g=Math.floor(Math.random()*d.length),k=d[g],l=k*c-e.x;l=0>l-e.width/2?e.width/2-e.x:l-e.width/2;l+e.width+2*e.x>b.width&&(l=b.width-e.width-e.x-50);var m=2048*(1-Math.floor(3*Math.random()))/5-e.y-e.height/2;1024<
m+e.y+e.height&&(m=974-e.height-e.y);d.splice(g,1);h.setAttribute("transform","translate("+l+" "+m+")");h.setAttribute("fill",u[k%u.length])}}bb()}}function $a(a){for(var b=0,c=0;c<a.length;c++){var d=void 0;var f="",h=new RegExp(Ca(a[c])+"\\t([^\\t]+)\\t([^\\t]+)\\t([^\\n]*)\n*");(d=window.stroke_parts_data.match(h))&&3<d.length&&(f=d[3]);d=f;""!==d?b+=Da(d,0).length:b++}return b}
function ab(a){var b=[],c;if("undefined"==typeof window.stroke_component||null==window.stroke_component)Pa();else{var d=new RegExp(Ca(a)+"\\t([^\\n]*)\n*");(c=window.stroke_component.match(d))&&2<=c.length?b=c[1].trim().split(";")[0].split(","):b.push(a)}return b}
function cb(a){var b=Q;"string"==typeof b&&(b=Array.from(b));var c="";b.forEach(function(d){return c+=(""==c?"":" AND")+" A LIKE '%"+d+"%'"});Ka("https://docs.google.com/spreadsheets/d/1gXpIr34zH65D50td6vnuKbM4yy1WPNQTbReDguCy5lY/edit?usp=sharing","SELECT * WHERE "+c,!0,!0,function(d){var f=[];if("undefined"!=typeof d&&null!=d&&"string"==typeof d.status&&"ok"==d.status){var h=d.table;if("undefined"!=typeof h.rows&&null!=h.rows&&1<h.rows.length&&(d=d.table.rows.filter(function(e){return e.c&&e.c[0].v.length==
b.length}),1<d.length))for("string"!=typeof b&&(b=b.join("")),h=0;h<d.length;h++)d[h].c[0].v!=b&&f.push(d[h].c[0].v.trim())}"function"==typeof a?a(f):console.log(JSON.stringify(f))})}function db(a){var b,c={x:0,y:0};(a=a.getAttribute("transform"))&&(b=a.match(/translate\s*\(([^\)]+)\)/i))&&(b=b[1].split(/\s+/))&&2==b.length&&(c.x=Number(b[0]),c.y=Number(b[1]));return c}
function eb(){var a=[],b,c=document.querySelector("#practiceSVG");if(c){for(var d=0;d<Q.length;d++)a[d]={},a[d].a=[],a[d].w={},a[d].cns=[];for(d=Q.length;d<c.children.length;d++){var f=c.children[d],h=f.getAttribute("id");if("g"==f.tagName&&"string"==typeof h&&(b=h.match(/([^-]+)-(\d+)-(\d+)/))){var e=b[1];h=f.getAttribute("cns");for(var g=0;g<a.length;g++)if(Qa(c.children[g],f)){a[g].a.push(f);a[g].cns.push(h);null==a[g].w[e]?a[g].w[e]=1:a[g].w[e]++;break}}}b=[];c=Q.slice();for(d=0;d<a.length;d++){f=
a[d].cns.length;e=Object.keys(a[d].w);for(g=0;g<e.length;g++)if(h=ab(e[g]),h.length!=f)delete a[d].w[e[g]];else{for(var k=0;k<f&&0<h.length;k++)for(var l=0;l<h.length;l++)if(h[l]==a[d].cns[k]){h.splice(l,1);break}0<h.length&&delete a[d].w[e[g]]}h=Object.keys(a[d].w).length;if(0==h)b.push(d),console.log("NG: ",d,a[d]);else if(1==h)for(k=0;k<c.length;k++)if(c[k]==Object.keys(a[d].w)[0]){c.splice(k,1);break}}for(d=0;d<b.length;d++)for(f=a[b[d]].cns.length,k=0;k<c.length;k++)if(e=c[k],h=ab(e),h.length==
...
function fb(){for(var a="",b=eb(),c=b.length-1,d=0;d<=c;d++){var f=b[d],h=Object.keys(f.w);h.sort(function(p,n){return f.w[p]-f.w[n]});for(var e=0;e<h.length;e++){var g=h[e],k=ab(g);if(f.cns.length==k.length){for(var l=0;l<f.cns.length&&0<k.length;l++)for(var m=0;m<k.length;m++)if(f.cns[l]==k[m]){k.splice(m,1);break}if(0==k.length){a+=g;break}}}}return{term:a,rect:b}}
function gb(a){function b(t,z){var C=null;if(t&&t.a&&1<t.a.length&&"string"==typeof z){t.a=t.a.sort(function(Ea,ub){return d(ub)-d(Ea)});for(var F=db(t.a[0]),K=0;K<t.a.length;K++){var G=t.a[K],y=G.getAttribute("id"),M=G.getAttribute("cns"),N=y.match(/([^-]+)-(\d+)-(\d+)/)[1],H=!0;M=p.querySelectorAll('g[id*="'+z+'"][cns="'+M+'"]');if(N==z){var D=db(G);204.8>=Math.abs(D.x-F.x)&&204.8>=Math.abs(D.y-F.y)&&(H=!1)}if(H&&0<M.length)for(N=0;N<M.length;N++)if(D=M[N],D.getAttribute("id")!=y&&(D=c(G,D),204.8>=
Math.abs(D.x-F.x)&&204.8>=Math.abs(D.y-F.y))){H=!1;break}if(H){C=G;break}}}return C}function c(t,z){var C=z.getBBox(),F=t.getBBox();z=F.x-C.x+(F.width-C.width)/2;C=F.y-C.y+(F.height-C.height)/2;t=db(t);t.x+=z;t.y+=C;return t}function d(t){t=t.getBBox();return t.width*t.height}function f(t){if(t){var z=t.getBBox();z=V("rect",{"class":"hintRect",width:z.width,height:z.height,x:z.x,y:z.y,style:"fill:#ffff00;fill-opacity:0.5;stroke:#ff0000;stroke-width:20;stroke-opacity:0.25"});t.insertBefore(z,t.children[0])}else(t=
document.querySelectorAll(".hintRect"))&&Array.from(t).forEach(function(C){return C.remove()})}var h=!0,e=null,g=0,k=0,l=[];if("string"==typeof a){var m=("string"==typeof Q?Q:Q.join("")).trim();a=a.trim();if(a.length>=m.length)l=new RegExp(Ca(m)),l.test(a)||("function"==typeof hb?(m=hb(m),a=hb(a),l=new RegExp(Ca(m)),l.test(a)||(h=!1)):h=!1);else return!1}else{Sa();for(m=0;m<Q.length;m++)l[m]={},l[m].q=ab(Q[m]),l[m].a=[],g+=l[m].q.length;var p=document.querySelector("#practiceSVG");if(p)for(m=0;m<
...
l[m],q=b(n,Q[m])){f(q);setTimeout(function(){f()},250);U("\u52a0\u6cb9<br>\u7b2c "+(m+1)+" \u500b\u5b57\u518d\u6392\u597d\u4e00\u9ede","#ff6699a0",.75);"undefined"!=typeof A&&null!=A&&W(A);return}}if(h||"string"==typeof a)ib(h,k,g,e);else if(a=fb(),Array.from(a.term).length==Q.length){console.log("search dic: "+a.term);U("\u5b57\u62fc\u5c0d\u4e86\uff0c\u8a5e\u4e0d\u540c<br>\uff5e\u67e5\u5b57\u5178\u4e2d\uff0c\u8acb\u7a0d\u5019\uff5e","#FF94B8",2);document.querySelector(".checkBtn").enable=!1;e="";
if(0<ua.length)for(m=0;m<ua.length;m++)if(a.term==ua[m]){e=a.term;break}if(""!=e)for(m=0;m<a.rect.length;m++)if(n=a.rect[m],q=b(n,Object.keys(a.rect[m].w)[0])){f(q);setTimeout(function(){f()},250);U("\u52a0\u6cb9<br>\u7b2c "+(m+1)+" \u500b\u5b57\u518d\u6392\u597d\u4e00\u9ede","#ff6699a0",.75);"undefined"!=typeof A&&null!=A&&W(A);return}ib(""!=e,k,g,e)}else ib(h,k,g,e);return h}
function ib(a,b,c,d){var f="";if(a){Wa();a=100;b="green";c=1.5;0==L.length?(f="\u5df2\u7d93\u5168\u90e8\u6311\u6230\u5b8c\uff5e\u52a05000\u5206",a=5E3,b="#56B4E9",c=3):f="\u606d\u559c\u904e\u95dc!!";U('<span style="color:#99cc33;font-size:1.25em;">\u592a\u68d2\u4e86</span><br>'+f,b,c,function(){var h=document.querySelector("#nextBtn");h&&h.classList.toggle("hidden",!1);jb(Q,d)});"undefined"!=typeof x&&null!=x&&W(x);f="";"string"!=typeof Q&&(f=Q.join(""));"string"==typeof d&&""!=d&&(f+=","+d);""!=
f&&Ta(f);f=[".checkBtn",".skipBtn",".randomBtn"];for(b=0;b<f.length;b++)(c=document.querySelector(f[b]))&&c.classList.toggle("hidden",!0);kb(a)}else f=b<c&&"string"!=typeof answerTxt?"\u8acb\u6aa2\u67e5\u90e8\u4ef6\u662f\u5426\u90fd\u5b8c\u6574\u5730\u5728\u65b9\u683c\u5167<br>\u6709 "+(c-b)+" \u500b\u9700\u8981\u8abf\u6574\u4f4d\u7f6e":"\u54e6~ \u54e6~~ \u518d\u8a66\u8a66",U('<span style="color:yellow;font-size:1.25em;">\u52a0\u6cb9</span><br>'+f,"red",3),"undefined"!=typeof A&&null!=A&&W(A)}
function lb(a){var b=document.createElement("textarea");b.innerHTML=a;return b.value}
...
function nb(a){"boolean"!=typeof a&&(a=!1);if("undefined"!=typeof O&&null!=O&&1<O.length){if("undefined"==typeof L||null==L||0==L.length){var b=O.split(/\n+/).filter(function(h){return""!=h.replace(/\s/g,"")});L=Array.from({length:b.length},function(h,e){return{q:b[e],p:$a(b[e]),H:!1}});L.sort(function(h,e){return h.p-e.p});ta=L.length;Y=L[0].p;mb=Y+1}var c=L[L.length-1].p;for(var d=[];Y<=c&&0<L.length;)if(d=L.filter(function(h){return h.p>=Y&&h.p<=mb&&!h.H}),0==d.length)Y++,mb++;else if(3<L.filter(function(h){return!h.H}).length&&
3>=d.length)mb++;else break;Y==L[0].p||L[0].H||(a||(U("\u606d\u559c\u5347\u7d1a<br>\u52a0\u767c\u9ede\u6578 1000","blue",2),kb(1E3)),Y=L[0].p);d=0<d.length?d.filter(function(h){return h.p==Y}):L.slice(0);var f=Math.floor(Math.random()*d.length);c=Array.from(d[f].q.trim());L=L.filter(function(h){return h.q!=d[f].q});"function"==typeof ob&&ob(ta-L.length)}return c}
function bb(){var a=document.querySelector("#practiceSVG"),b=Array.from(a.children);b.splice(0,Q.length);b.sort(function(c,d){c=c.getBBox();d=d.getBBox();return d.width*d.height-c.width*c.height});b.forEach(function(c){return c.remove()});b.forEach(function(c){return a.appendChild(c)})}
function pb(){var a=document.querySelector("#practice"),b=document.querySelector("#practiceSVG");if(b&&a&&"undefined"!=typeof Q&&null!=Q){var c=a.getBoundingClientRect();a=c.width/Q.length;c=c.height/2.25;var d=1;a>c&&(d=a/c);b.setAttribute("viewBox","0 0 "+Math.floor(2048*Q.length*d+40*(Q.length+1))+" 2048");b.setAttribute("width","100%");b.setAttribute("height","100%")}}
function qb(a){"boolean"!=typeof a&&(a=!1);Array.from(document.querySelector("#practiceSVG").children).forEach(function(c){return c.remove()});var b=document.querySelector("#nextBtn");b&&b.classList.toggle("hidden",!0);jb();Q=nb(a);"undefined"!=typeof Q&&null!=Q&&0<Q.length&&("string"==typeof Q&&(Q=Array.from(Q.trim())),cb(function(c){ua=c}),pb(),Xa(),P=Array.from({length:Q.length},function(c,d){return d}),Ya(function(){bb();for(var c=[".checkBtn",".skipBtn",".randomBtn"],d=0;d<c.length;d++){var f=
document.querySelector(c[d]);f&&f.classList.toggle("hidden",!1)}sa&&(U('<span style="color:#ffff00;font-size:1.25em;">\u958b\u59cb\u95d6\u95dc</span><br>\u8acb\u5c07\u90e8\u4ef6\u62d6\u66f3\u5230\u65b9\u6846\u4e2d\u62fc\u6210\u5b57<br>\u8a9e\u8a5e\u62fc\u597d\u4e86\uff0c\u5c31\u6309 [\u9001\u51fa\u7b54\u6848] \u9215',"orange",3),sa=!1)}))}var Z=null;function rb(){var a=document.getElementById("micButton");a&&(a.style["background-color"]="dodgerblue");Z&&"function"==typeof Z.stop&&Z.stop()}
function sb(){Z.onstart=function(){document.getElementById("micButton").style["background-color"]="Violet";console.log("--------\u8a9e\u97f3\u8fa8\u8b58\u958b\u59cb--------")};Z.onend=function(){var a=document.getElementById("micButton");a.style["background-color"]="DodgerBlue";a.o.innerText="";console.log("========\u8a9e\u97f3\u8fa8\u8b58\u7d50\u675f========")};Z.onresult=function(a){var b=a.resultIndex;a=a.results[b][a.results[b].length-1].transcript;console.log(a);b=/Viole/i.test(document.getElementById("micButton").style["background-color"]);
a&&b&&(b=300,gb(a)?rb():b=1E3,document.getElementById("micButton").o.innerText=a,setTimeout(function(){document.getElementById("micButton").o.innerText=""},b))};Z.onerror=function(a){var b="\u932f\u8aa4\u4ee3\u78bc\u53ca\u8a0a\u606f: ["+a.error+"] ["+a.message+"]";/not-allowed/i.test(a.error)?U("\u7121\u6cd5\u4f7f\u7528\u8a9e\u97f3\u8fa8\u8b58(\u53ef\u80fd\u9700\u8981\u6aa2\u67e5\u9ea5\u514b\u98a8\u8a2d\u5b9a\u3001\u5b89\u5168\u3001\u96b1\u79c1\u8a2d\u5b9a\uff1b\u6216\u662f\u8acb\u63db\u4e00\u7a2e\u700f\u89bd\u5668\u518d\u8a66\u8a66)<br>"+
...
"undefined"!=typeof l&&null!=l?"string"==typeof l.v&&""!=l.v.replace(/\s/g,"")?h+=l.v.trim()+"\n":d||(h+="\n"):d||(h+="\n")}""!=h?"function"==typeof f?f(h):console.log(h):("undefined"!=typeof g&&null!=g&&"undefined"!=typeof g.table&&console.log(g.table),R=!1,U("\u7121\u6cd5\u8f09\u5165\u984c\u5eab<br>\u8acb\u78ba\u8a8d\u8a66\u7b97\u8868\u4e2d\u7684\u5167\u5bb9\u5f8c\u518d\u8a66","red",10,function(){zb(0)}))}})}else R=!1,U("\u7121\u6cd5\u8f09\u5165\u984c\u5eab<br>\u7db2\u5740\u4f3c\u4e4e\u4e0d\u662f\u8a66\u7b97\u8868\u7684\uff0c\u8acb\u78ba\u8a8d\u5f8c\u518d\u8a66",
"red",10,function(){zb(0)})}function Ab(a,b,c,d,f){function h(g){e||(e=g);g=Math.min((g-e)/d,1);a.innerHTML=Math.floor(g*(c-b)+b);1>g?window.requestAnimationFrame(h):"function"==typeof f&&f()}var e=null;window.requestAnimationFrame(h)}function kb(a){function b(){"undefined"!=typeof E&&null!=E&&W(E)}var c=document.querySelector(".score");if("number"!=typeof a)c.innerHTML="0",b();else{var d=parseInt(c.innerHTML,10);Ab(c,d,d+a,50>a?500:1E3,function(){b()})}}
function ob(a){var b=document.querySelector(".q-number"),c="^_^";b&&("number"==typeof a&&(c=a+" / "+ta),b.innerHTML=c)}
function jb(a,b){var c=document.querySelector(".dicLink");c&&("undefined"==typeof a||null==a?a="":("string"!=typeof a&&(a=a.join("")),"undefined"==typeof b||null==b?a='<a href="https://pedia.cloud.edu.tw/Entry/Detail?title='+encodeURIComponent(a)+'#" target="_blank">\u67e5\u770b\u6559\u80b2\u96f2-\u6559\u80b2\u767e\u79d1 ['+a+"] \u7684\u89e3\u91cb</a>":(a='\u67e5\u770b\u6559\u80b2\u96f2-\u6559\u80b2\u767e\u79d1\u7684\u89e3\u91cb &gt;&gt; [<a href="https://pedia.cloud.edu.tw/Entry/Detail?title='+(encodeURIComponent(a)+
...
function Oa(){if("undefined"!=typeof window.stroke_parts_data&&null!=window.stroke_parts_data){document.querySelector(".loading-label").innerHTML="\u8f09\u5165\u984c\u5eab";Ua();var a=X("id"),b=X("gid"),c=X("col"),d="https://docs.google.com/spreadsheets/d/1kBueULlojPOH9E3EZYEUcUAv1HfJm_wULQT1hT2m1nM/edit?usp=sharing";""==a.replace(/\s/g,"")?""!=b&&(d=/gid=/.test(d)?d.replace(/gid=([^&\?#]+)/g,"gid="+b):d+((/\?/.test(d)?"&":"?")+"gid="+b+"#"+b)):d="https://docs.google.com/spreadsheets/d/"+a+"/edit?"+
(""==b?"usp=sharing":"?gid="+b+"#"+b);a=null;""!=c.replace(/\s|[^a-z]/ig,"")&&(a="SELECT "+c.toUpperCase());Ka(d,a,!1,!1,function(f){R=!1;O=f.trim();var h=parseInt(X("lesson"),10);isNaN(h)||(f=O.split(/\n\n+/),0<=h&&h<=f.length&&(O=f[h-1]));sa=!0;qb()});wb()}else setTimeout(function(){R=!1;U("HTML5 FUN \u90e8\u4ef6\u8a2d\u5b9a\u8f09\u5165\u5931\u6557<br>\u8acb\u6aa2\u67e5\u5f8c\u518d\u8a66","red",5,function(){zb(0)})},500)}
function Fb(){function a(){"undefined"!=typeof window.stroke_component&&null!=window.stroke_component?"undefined"==typeof window.stroke_parts_data||null==window.stroke_parts_data?Na():Oa():setTimeout(function(){U("CNS \u90e8\u4ef6\u8a2d\u5b9a\u8f09\u5165\u5931\u6557<br>\u8acb\u6aa2\u67e5\u5f8c\u518d\u8a66","red",5,function(){zb(0)});R=!1},500)}document.querySelector(".loading-label").innerHTML="\u8f09\u5165\u8a2d\u5b9a";"undefined"==typeof window.stroke_component||null==window.stroke_component?Pa(a):
a()}function Bb(a){for(var b=3;10>=b;b++)try{document.querySelector("#aswift_"+b).parentElement.parentElement.style.scale=a}catch(c){}}function Gb(){try{"function"==typeof Bb&&Bb(.001)}catch(a){}Cb();Db();zb(!0);window.scrollTo(0,0);document.querySelector(".debug").innerHTML="";R=!0;Eb();Sa();Ta("\u52a0\u6cb9");setTimeout(function(){kb();ob();jb()},100);Fb()}var Hb=X("autostart");"1"!=Hb&&"true"!=Hb||Gb();window.setVisibility=zb;
window.btnSleep=function(a,b){a.disabled=!0;setTimeout(function(){a.disabled=!1},b)};window.newGame=qb;window.showAnswer=Wa;window.checkAnswer=gb;window.skipQuestion=function(){if(0<L.length){var a=Q.join(""),b=$a(a);L.push({q:a,p:b,H:!0});a=["\u56de\u982d\u898b","\u5f85\u6703\u5152\u898b","See you later"];U("\uff5e"+a[Math.floor(Math.random()*a.length)]+"\uff5e","#00B0F0",.6);qb(!0)}else U("\u8acb\u52c7\u6562\u9762\u5c0d<br>\u9019\u662f\u6700\u5f8c\u4e00\u984c\u4e86","red")};
window.randomPosition=Za;
